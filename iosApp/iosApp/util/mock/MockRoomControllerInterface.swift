...
class MockRoomControllerInterfrace: RoomControllerInterface {
    ...
    func setAutoRevealVotes(autoReveal: Bool) {
        print("autoRevealVotes set to \(autoReveal)")
    }
    ...
    func getMockRoomData() -> RoomData {
        return RoomData(configuration:
                            RoomConfiguration(url: MultiUrl(base: "mockUrl.mock/mockRoomId"),roomJoinCode: "2137", isOwner: true, scale: ["1", "2", "3", "4", "5", "?"], scaleType: "MOCK1", scaleTypeList: ["MOCK1", "MOCK2"], voteEnded: false, anonymousVote: true, alwaysVisibleVote: true, autoRevealVotes: true),
